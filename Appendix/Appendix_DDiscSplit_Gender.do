...
forvalues g = 0/1 { // note: this is the value for *focal gender* not index gender 
	***** Regressions using *residuals* of insnf
	use  "$input_datapath/RDdata.dta", clear 
	
	bys response_id: ereplace fem = max(fem) 
	keep if fem == `g'
...
texdoc init "$hoaglandoutput/D-DiscTable_byGender.tex", replace force
tex \begin{table}[htb]
tex \centering
...
tex \toprule
tex & (1) & (2) & (3) \\
tex &  & Wives' Health Shock Affecting & Husbands' Health Shock Affecting  \\ 
tex & All & Focal Husbands & Focal Wives \\ 
tex \midrule
